...
try:
    ...
    if lt_escolhida:
        ...
        if plotar_clicado and lt_escolhida in abas and valor_busca > 0:
            ...
            km_col = "km"
            # Coluna D deve ser a descrição da localização/Torre
            ...
            if len(torre_idx) > 0:
                ...
                current_code = "" # Código da torre (Coluna C da LT)
                
                for index, row in df_plot.iterrows():
                    x = row["x_pos"]
                    # Lendo o código da torre (ex: T2B) da coluna de fases, que está na Coluna C
                    raw_seq_or_code = str(row[fase_seq_col]).strip().upper() 
                    
                    seq_fase_real = raw_seq_or_code 
                    
                    # Lemos o label da torre diretamente da Coluna D da LT
                    ...
                    if is_brasnorte and raw_seq_or_code in torres_jbju_map:
                        # Acha a sequência de fases real (A-C-B) e o caminho da imagem usando o código da torre (ex: T2B)
                    ...
                    # Atualiza informações para a torre central
                    ...
                    # Adiciona pontos para o gráfico de sequências (usa a sequência real/mapeada)
                ...
                with col_fig:
                    ...
                    for index, row in df_plot.iterrows():
                        ...
                        # Obtém informações para exibição (usando a Coluna D como rótulo)
                ...
                with col_imagem:
                    ...
                    if imagem_torre_central_excel and imagem_torre_central_excel.strip():
                        ...
                        # Normaliza para usar o nome do arquivo, garantindo que o prefixo 'img' seja usado.
                        caminho_completo_tentativa = os.path.normpath(os.path.join("img", os.path.basename(caminho_excel)))
                        
                        # Tenta com o nome exato (caso o usuário tenha digitado 'img\nome.png' no Excel)
                        caminho_alternativo = os.path.normpath(caminho_excel)


                        # Tentativa de carregar a imagem
                        imagem_carregada = False
                        
                        # Tenta 1: Assumindo que o Excel tem apenas o nome do arquivo e queremos 'img/nome'
                        if os.path.exists(caminho_completo_tentativa):
                            st.image(caminho_completo_tentativa, caption=f"Torre {current_code}")
                            imagem_carregada = True
                        
                        # Tenta 2: Assumindo que o Excel já tem o caminho completo e/ou o nome exato (com ou sem barras)
                        elif os.path.exists(caminho_alternativo):
                            st.image(caminho_alternativo, caption=f"Torre {current_code}")
                            imagem_carregada = True

                        if not imagem_carregada:
                            st.warning(f"❌ Arquivo não encontrado. Verifique se o arquivo está na pasta `img` e se o nome está correto. Tentativas: `{caminho_completo_tentativa}` ou `{caminho_alternativo}` não foram localizados.")
